...
def analyze_simplified_api(
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
            shutil.copyfileobj(resume_file.file, temp_pdf)
            pdf_path = temp_pdf.name

        # Generate the report
        generate_report_with_background(pdf_path, position, candidate_name, background_path, chapter)
        
        # Return the generated PDF
        return FileResponse(
            f"Reporte_analisis_cargo_{candidate_name}_{position}_{chapter}.pdf",
        ...
        st.error(f"An error occurred: {e}") # This won't work in FastAPI
        return JSONResponse(content={"error": str(e)}, status_code=500)
    finally:
        # Ensure the temporary file is deleted
        os.remove(pdf_path)

def analyze_descriptive_api(
    candidate_name: str, 
    position: str,
    chapter: str,
    resume_file: UploadFile = File(...)
    ...
    try:
        ...
        analyze_and_generate_descriptive_report_with_background(pdf_path, position, candidate_name, advice, indicators, background_path, chapter)
        
        return FileResponse(
            f"Reporte_descriptivo_cargo_{candidate_name}_{position}_{chapter}.pdf",
            media_type="application/pdf",
